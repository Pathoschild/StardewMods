...
internal class FishSpawnRulesField : CheckboxListField
{
    ...
    *********/
    ...
    public FishSpawnRulesField(GameHelper gameHelper, string label, ParsedItemData fish)
        : this(label, [new CheckboxList(GetConditions(gameHelper, fish))])
    {
    }
    ...
    public FishSpawnRulesField(GameHelper gameHelper, string label, GameLocation location, Vector2 tile, string fishAreaId)
        : this(label, GetConditions(gameHelper, location, tile, fishAreaId))
    {
    }
    ...
    public FishSpawnRulesField(string label, IEnumerable<CheckboxList> spawnConditions)
    : base(label)
    {
        this.CheckboxLists = spawnConditions.ToArray();
    ...
    *********/
    ...
    private static IEnumerable<CheckboxList> GetConditions(GameHelper gameHelper, GameLocation location, Vector2 tile, string fishAreaId)
    {
        foreach (FishSpawnData spawnRules in gameHelper.GetFishSpawnRules(location, tile, fishAreaId))
        {
            ...
            CheckboxList checkboxList = new(GetConditions(gameHelper, fishItemData));
    ...
    private static IEnumerable<Checkbox> GetConditions(GameHelper gameHelper, ParsedItemData fish)
    {
        ...
        if (spawnRules.IsUnique)
            yield return GetCondition(I18n.Item_FishSpawnRules_NotCaughtYet(), !Game1.player.fishCaught.ContainsKey(fish.QualifiedItemId));

        // fishing level
        if (spawnRules.MinFishingLevel > 0)
            yield return GetCondition(I18n.Item_FishSpawnRules_MinFishingLevel(level: spawnRules.MinFishingLevel), Game1.player.FishingLevel >= spawnRules.MinFishingLevel);

        // extended family quest
        if (spawnRules.IsLegendaryFamily)
            yield return GetCondition(I18n.Item_FishSpawnRules_ExtendedFamilyQuestActive(), Game1.player.team.SpecialOrderRuleActive("LEGENDARY_FAMILY"));

        // weather
        if (spawnRules.Weather == FishSpawnWeather.Sunny)
            yield return GetCondition(I18n.Item_FishSpawnRules_WeatherSunny(), !Game1.isRaining);
        else if (spawnRules.Weather == FishSpawnWeather.Rainy)
            yield return GetCondition(I18n.Item_FishSpawnRules_WeatherRainy(), Game1.isRaining);

        // time of day
        if (spawnRules.TimesOfDay?.Any() == true)
        {
            yield return GetCondition(
        ...
        // locations & seasons
        if (HaveSameSeasons(spawnRules.Locations))
        {
            var firstLocation = spawnRules.Locations[0];

            // seasons
            if (firstLocation.Seasons.Count == 4)
                yield return GetCondition(I18n.Item_FishSpawnRules_SeasonAny(), true);
            else
            {
                yield return GetCondition(
        ...
        else
        {
            ...
            yield return GetCondition(summary, hasMatch);
        }
    }
