        ...
        private IEnumerable<Entry> GetRecipeEntries(GameHelper gameHelper, Item inputItem, IEnumerable<RecipeModel> recipes)
        {
            foreach (RecipeModel recipe in recipes)
            {
                ...
                RecipeIngredientModel ingredient =
                    recipe.Ingredients.FirstOrDefault(p => p.Ids.Contains(inputItem.ParentSheetIndex) && p.Matches(inputItem))
                    ?? recipe.Ingredients.FirstOrDefault(p => p.Ids.Contains(inputItem.Category) && p.Matches(inputItem));
