...
namespace ContentPatcher.Framework
{
    ...
    internal class PatchLoader
    {
        ...
        *********/
        ...
        public PatchLoader(PatchManager patchManager, TokenManager tokenManager, IMonitor monitor, IImmutableSet<string> installedMods, Func<string, IAssetName> parseAssetName)
        {
        ...
        public bool TryParseConditions(IDictionary<string, string?>? raw, TokenParser tokenParser, LogPathBuilder path, out Condition[] conditions, out IImmutableSet<string> immutableRequiredModIDs, [NotNullWhen(false)] out string? error)
        {
            ...
            {
                if (!this.TryParseCondition(key, value, tokenParser, path.With(key), out Condition? condition, ref immutableRequiredModIDs, out error))
                {
                    conditions = Array.Empty<Condition>();
                    return false;
                }
        ...
        *********/
        ...
        private bool TryParseTextOperations(PatchConfig patch, TokenParser tokenParser, IImmutableSet<string> assumeModIds, LogPathBuilder path, out IList<TextOperation> textOperations, [NotNullWhen(false)] out string? error)
        {
        ...
        /// <param name="error">The error message indicating why parsing failed, if applicable.</param>
        /// <returns>Returns whether parsing succeeded.</returns>
        private bool TryParseEditDataFields(PatchConfig entry, TokenParser tokenParser, IImmutableSet<string> assumeModIds, LogPathBuilder path, out List<EditDataPatchRecord> entries, out List<EditDataPatchField> fields, out List<EditDataPatchMoveRecord> moveEntries, out List<IManagedTokenString> targetField, [NotNullWhen(false)] out string? error)
        {
        ...
        /// <param name="error">An error message indicating why normalization failed.</param>
        private bool TryParseCondition(string? name, string? value, TokenParser tokenParser, LogPathBuilder path, [NotNullWhen(true)] out Condition? condition, ref IImmutableSet<string> immutableRequiredModIDs, [NotNullWhen(false)] out string? error)
        {
            ...
            if (condition.IsReady && !condition.IsMutable && condition.Is(ConditionType.HasMod))
            {
                ...
                if (condition.Input.ReservedArgs.TryGetValue(InputArguments.ContainsKey, out IInputArgumentValue? contains))
                {
                    if (bool.TryParse(condition.Values.Value, out bool required) && required)
                        immutableRequiredModIDs = immutableRequiredModIDs.AddMany(contains.Parsed);
                }

                // values
                else
                    immutableRequiredModIDs = immutableRequiredModIDs.AddMany(condition.CurrentValues);
            }
        ...
        private bool TryParseBoolean(string? rawValue, TokenParser tokenParser, IImmutableSet<string> assumeModIds, LogPathBuilder path, [NotNullWhen(false)] out string? error, [NotNullWhen(true)] out IManagedTokenString? parsed)
        {
            // analyze string
            ...
            if (parsed.HasAnyTokens)
            {
                ...
                {
                    if (!token.HasBoundedValues(input, out IImmutableSet<string>? allowedValues) || !allowedValues.All(p => bool.TryParse(p, out _)))
                    {
        ...
        /// <param name="error">An error phrase indicating why parsing failed (if applicable).</param>
        /// <param name="parsed">The parsed value.</param>
        private bool TryParseEnabled(string? rawValue, TokenParser tokenParser, IImmutableSet<string> assumeModIds, LogPathBuilder path, [NotNullWhen(false)] out string? error, out bool parsed)
        {
            parsed = false;
        ...
        /// <param name="error">An error phrase indicating why parsing failed (if applicable).</param>
        /// <param name="parsed">The parsed value.</param>
        private bool TryParsePosition(PatchPositionConfig? raw, TokenParser tokenParser, IImmutableSet<string> assumeModIds, LogPathBuilder path, [NotNullWhen(false)] out string? error, [NotNullWhen(true)] out TokenPosition? parsed)
        {
        ...
        /// <param name="error">An error phrase indicating why parsing failed (if applicable).</param>
        /// <param name="parsed">The parsed value.</param>
        private bool TryParseRectangle(PatchRectangleConfig raw, TokenParser tokenParser, IImmutableSet<string> assumeModIds, LogPathBuilder path, [NotNullWhen(false)] out string? error, [NotNullWhen(true)] out TokenRectangle? parsed)
        {
            bool TryParseField(string? rawField, string name, [NotNullWhen(true)] out IManagedTokenString? result, [NotNullWhen(false)] out string? parseError)
            {
                if (!this.TryParseInt(rawField, tokenParser, assumeModIds, path.With(name), out parseError, out result))
        ...
        /// <param name="error">An error phrase indicating why parsing failed (if applicable).</param>
        /// <param name="parsed">The parsed value.</param>
        private bool TryParseInt(string? rawString, TokenParser tokenParser, IImmutableSet<string> assumeModIds, LogPathBuilder path, [NotNullWhen(false)] out string? error, [NotNullWhen(true)] out IManagedTokenString? parsed)
        {
            parsed = null;
            ...
            if (tokenString.HasAnyTokens)
            {
                ...
                if (!token.BypassesContextValidation)
                {
                    bool isIntegerBounded =
                        ...
                        || (token.HasBoundedValues(input, out IImmutableSet<string>? allowedValues) && allowedValues.All(p => int.TryParse(p, out _)));
        ...
        private bool TryPrepareLocalAsset(string? path, TokenParser tokenParser, IImmutableSet<string> assumeModIds, LogPathBuilder logPath, [NotNullWhen(false)] out string? error, [NotNullWhen(true)] out IManagedTokenString? tokenedPath)
        {
            // normalize raw value
            path = path?.Trim();
