...
namespace ContentPatcher.Framework
{
    ...
    public class ContentPatcherAPI : IContentPatcherAPI
    {
        ...
        *********/
        ...
        /// <param name="getValue">A function which returns the current value for the given input arguments (if any). For example, <c>{{your-mod-id/PlayerInitials}}</c> would result in a null input; <c>{{your-mod-id/PlayerInitials:{{PlayerName}}}}</c> would pass in the parsed string after token substitution, like <c>"John Smith"</c>. If the token doesn't use input arguments, you can simply ignore the input.</param>
        /// <param name="allowsInput">Whether the player can provide input arguments (see <paramref name="getValue"/>).</param>
        /// <param name="requiresInput">Whether the token can *only* be used with input arguments (see <paramref name="getValue"/>).</param>
        [Obsolete]
