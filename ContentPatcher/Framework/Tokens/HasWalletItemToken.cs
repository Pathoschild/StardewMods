    ...
    internal class HasWalletItemToken : BaseToken
    {
        ...
        *********/
        ...
        public override IEnumerable<string> GetValues(TokenName name)
        {
            ...
            if (name.HasSubkey())
            {
                bool hasItem = Enum.TryParse(name.Subkey, true, out WalletItem item) && this.WalletItems[item]();
        ...
        *********/
        ...
        private IEnumerable<string> GetInvalidValues(IEnumerable<string> values)
        {
            ...
            {
                if (!Enum.TryParse(value, true, out WalletItem item) || !Enum.IsDefined(typeof(WalletItem), item) || !this.WalletItems.ContainsKey(item))
                    yield return value;
            }
        }
    }
