...
`PatchMode` | <p>How to apply `FromArea` to `ToArea`. Defaults to `Replace`.</p> Possible values: <ul><li><code>Replace</code>: replace every pixel in the target area with your source image. If the source image has transparent pixels, the target image will become transparent there.</li><li><code>Overlay</code>: draw your source image over the target area. If the source image has transparent or semi-transparent pixels, the target image will 'show through' those pixels. Opaque pixels will replace the target pixels.</li><li><p>`Mask`: apply the source image as a _transparency mask_, which subtracts the opacity of each pixel in the mask from the same pixel in the target image. For example, a fully opaque pixel in the new content results in a fully transparent pixel in the final image. (Colors in the mask image are ignored; only transparency matters.)</p><p>The `Mask` mode is often used to delete specific areas in a spritesheet before editing in new sprites, which is much more efficient than having a separate edit for each sprite.</p></li></ul>For example, let's say your source image is a pufferchick with a transparent background, and the target image is a solid green square. Here's how they'll be combined with different `PatchMode` values:<br />![](../screenshots/patch-mode-examples.png)
